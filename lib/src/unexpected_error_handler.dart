...
/// Widget that listens to the events and states that implement [UnexpectedErrorAPI] in specified the [BLOC].
...
class UnexpectedErrorHandler<BLOC extends SafeBlocBase<STATE>, STATE> extends StatelessWidget {
  ...
  /// Callback that returns a [Widget] that is displayed if state that implements [UnexpectedErrorAPI] is emitted by [BLOC].
  final UnexpectedErrorWidgetBuilder? errorScreen;

  /// Callback that is invoked if event that implements [UnexpectedErrorAPI] is emitted by [BLOC].
  ...
  void _handleError(BuildContext context, BaseEffect effect) {
    // BaseEffect can implement UnexpectedErrorAPI
    // ignore: avoid-unrelated-type-assertions
    if (effect case final UnexpectedErrorAPI error) {
